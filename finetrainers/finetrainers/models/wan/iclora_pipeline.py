...
from diffusers.utils import is_ftfy_available, is_torch_xla_available, logging, replace_example_docstring, export_to_video
...
class WanICLoRAPipeline(DiffusionPipeline, WanLoraLoaderMixin):
    r"""
    ...
    def encode_video_to_latents(self, video: torch.Tensor) -> torch.Tensor:
        """Encode video tensor to normalized latents.
        
        Args:
            video: Video tensor in [B, F, C, H, W] or [B, C, F, H, W] format
        
        Returns:
            Normalized latents in [B, C, F, H, W] format
        """
        ...
        # Check if input is [B, F, C, H, W] (channel at dim 2) and convert if needed
        # Typically channels=3, so if dim 1 != 3 and dim 2 == 3, it's [B, F, C, H, W]
        if video.shape[1] != 3 and video.shape[2] == 3:
            # Input is [B, F, C, H, W], convert to [B, C, F, H, W]
            video = video.permute(0, 2, 1, 3, 4).contiguous()
        # If already [B, C, F, H, W] (channel at dim 1), no conversion needed
        ...
        # Normalize
        latents_mean = torch.tensor(self.vae.config.latents_mean).view(1, self.vae.config.z_dim, 1, 1, 1).to(device=device, dtype=torch.float32)
        latents_std = (1.0 / torch.tensor(self.vae.config.latents_std)).view(1, self.vae.config.z_dim, 1, 1, 1).to(device=device, dtype=torch.float32)
        latents = ((latents.float() - latents_mean) * latents_std).to(latents.dtype)
        
        return latents

    def decode_latents_to_video(self, latents: torch.Tensor, output_type: str = "np") -> torch.Tensor:
        """Decode normalized latents to video tensor."""
        latents = latents.to(self.vae.dtype)
        
        # Denormalize
        # Follow the same pattern as attention.md: latents_std = 1/std, then latents / latents_std + mean
    ...
    def __call__(
        ...
        if not output_type == "latent":
            video = self.decode_latents_to_video(latents, output_type=output_type)
        else:
            video = latents
