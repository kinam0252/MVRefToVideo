...
TRAINING_DATASET_CONFIG="examples/training/sft/wan/mvref_lora_sdedit/training.json"
VALIDATION_DATASET_FILE="examples/training/sft/wan/mvref_lora_sdedit/validation.json"
...
model_cmd=(
  ...
  --use_iclora
  --iclora_mode "sdedit"  # SDEdit mode: skip ICLoRA processing during training
)
...
training_cmd=(
  ...
  --seed 42
  --batch_size 1
  --train_steps 10000 #테스트 -> 100, 실제 -> 3000
  --rank $RANK
  --lora_alpha $RANK
...
#AFTER 
# SDEdit 모드용 출력 디렉토리
OUTPUT_DIR="outputs/wan_iclora_sdedit_r${RANK}"
